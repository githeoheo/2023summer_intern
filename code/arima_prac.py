...
# cycle_df = cycle_df.rename(columns={'Cycle_Curve':'Cycle'})
...
forecast_year = int(len(cycle_df) * 0.2)
...
for i in range(5):
    ...
    model = ARIMA(df_train["Cycle"], order=optimal[0][0])
    model_fit = model.fit()
    ...
    model = SARIMAX(cycle_df, order=optimal[0][0][0], seasonal_order=optimal[0][0][1])
    model_fit = model.fit(disp=0)
    forecast_s = model_fit.forecast(steps=forecast_year)
